...
static NSString *const FailedUpdatesKey = @"BUNDLE_ROCKET_FAILED_UPDATES";
static NSString *const PendingUpdateKey = @"BUNDLE_ROCKET_PENDING_UPDATE";
...
{
    NSError *error;
    NSString *packageFile = [BundleRocketPackage getCurrentPackageBundlePath:&error];
    ...
    NSString *binaryAppVersion = [[BundleRocketConfig current] appVersion];
    NSDictionary *currentPackageMetadata = [BundleRocketPackage getCurrentPackage:&error];
...
/*
 * This returns a boolean value indicating whether BundleRocket has
...
+ (void)clearUpdates
{
    [BundleRocketPackage clearUpdates];
 ...
 * to point at the latest BundleRocket update, and then restarts
...
- (void)loadBundle
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([BundleRocket isUsingTestConfiguration] || ![_bridge.bundleURL.scheme hasPrefix:@"http"]) {
            _bridge.bundleURL = [BundleRocket bundleURL];
        }
...
{
    NSError *error;
    NSString *packageHash = [BundleRocketPackage getCurrentPackageHash:&error];
    
    // Write the current package's hash to the "failed list"
    [self saveFailedUpdate:packageHash];
    
    // Rollback to the previous version and de-register the new update
    [BundleRocketPackage rollbackPackage];
    [BundleRocket removePendingUpdate];
    [self loadBundle];
}
...
{
    [BundleRocketPackage downloadPackage:updatePackage
     ...
     // The download completed
                        doneCallback:^{
                            NSError *err;
                            NSDictionary *newPackage = [BundleRocketPackage getPackage:updatePackage[PackageHashKey] error:&err];
...
/*
 * This is the native side of the BundleRocket.getConfiguration method. It isn't
 * currently exposed via the "react-native-code-push" module, and is used
 * internally only by the BundleRocket.checkForUpdate method in order to get the
 * app version, as well as the deployment key that was configured in the Info.plist file.
 */
RCT_EXPORT_METHOD(getConfiguration:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    resolve([[BundleRocketConfig current] configuration]);
}

/*
 * This method is the native side of the BundleRocket.getCurrentPackage method.
 */
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    dispatch_async(dispatch_get_main_queue(), ^{
        NSError *error;
        NSMutableDictionary *package = [[BundleRocketPackage getCurrentPackage:&error] mutableCopy];
...
RCT_EXPORT_METHOD(installUpdate:(NSDictionary*)updatePackage
                  installMode:(BundleRocketInstallMode)installMode
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *error;
        [BundleRocketPackage installPackage:updatePackage
                                  error:&error];
        ...
        if (error) {
            ...
            if (installMode == BundleRocketInstallModeOnNextResume && !_hasResumeListener) {
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    && [packageHash isEqualToString:[BundleRocketPackage getCurrentPackageHash:&error]];
    
    resolve(@(isFirstRun));
}

/*
 * This method is the native side of the BundleRocket.notifyApplicationReady() method.
 */
...
/*
 * This method is the native side of the BundleRocket.restartApp() method.
 */
...
/*
 * This method is the native side of the BundleRocket.downloadAndReplaceCurrentBundle()
...
{
    if ([BundleRocket isUsingTestConfiguration]) {
        [BundleRocketPackage downloadAndReplaceCurrentBundle:remoteBundleUrl];
    }
}
