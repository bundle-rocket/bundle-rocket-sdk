...
@implementation BundleRocketPackage

NSString * const BundleRocketErrorDomain = @"BundleRocketError";
const int BundleRocketErrorCode = -1;
NSString * const DiffManifestFileName = @"hotbundlerocket.json";
NSString * const DownloadFileName = @"download.zip";
NSString * const RelativeBundlePathKey = @"bundlePath";
NSString * const StatusFile = @"bundlerocket.json";
NSString * const UpdateBundleFileName = @"app.jsbundle";
NSString * const UnzippedFolderName = @"unzipped";

+ (NSString *)getBundleRocketPath
{
    NSString* bundleRocketPath = [[BundleRocket getApplicationSupportDirectory] stringByAppendingPathComponent:@"BundleRocket"];
    if ([BundleRocket isUsingTestConfiguration]) {
        bundleRocketPath = [bundleRocketPath stringByAppendingPathComponent:@"TestPackages"];
    }

    return bundleRocketPath;
}

+ (NSString *)getDownloadFilePath
{
    return [[self getBundleRocketPath] stringByAppendingPathComponent:DownloadFileName];
}

+ (NSString *)getUnzippedFolderPath
{
    return [[self getBundleRocketPath] stringByAppendingPathComponent:UnzippedFolderName];
}

+ (NSString *)getStatusFilePath
{
    return [[self getBundleRocketPath] stringByAppendingPathComponent:StatusFile];
}
...
{
    NSString *folderPath = [BundleRocketPackage getCurrentPackageFolderPath:error];
    if (!*error) {
        if (!folderPath) {
...
{
    return [[self getBundleRocketPath] stringByAppendingPathComponent:packageHash];
}
           ...
           failCallback:(void (^)(NSError *err))failCallback
{
    ...
    BundleRocketDownloadHandler *downloadHandler = [[BundleRocketDownloadHandler alloc]
        init:downloadFilePath
        progressCallback:progressCallback
        doneCallback:^(BOOL isZip) {
            NSError *error = nil;
            NSString * unzippedFolderPath = [BundleRocketPackage getUnzippedFolderPath];
            ...
            if (isZip) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:diffManifestFilePath]) {
                    ...
                    [BundleRocketPackage copyEntriesInFolder:currentPackageFolderPath
                ...
                [BundleRocketPackage copyEntriesInFolder:unzippedFolderPath
                                          destFolder:newPackageFolderPath
                                               error:&error];
                ...
                if (relativeBundlePath) {
                    ...
                    error = [[NSError alloc] initWithDomain:BundleRocketErrorDomain
                                                       code:BundleRocketErrorCode
                                                   userInfo:@{
...
+ (void)clearUpdates
{
    [[NSFileManager defaultManager] removeItemAtPath:[self getBundleRocketPath] error:nil];
